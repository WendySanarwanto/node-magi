...
var magi = require('../lib/magi')(options);
...
exports.get = function (test) {
	...
	for(var propt in options){
		test.deepEqual(magi.get(''+options_keys[idx]), options[''+propt]);
		idx ++;
	}
...
exports.set = function (test) {
	...
	for(var propt in new_options){
		magi.set(''+options_keys[idx], new_options[ ''+options_keys[idx] ]);
		test.deepEqual(magi.get(''+options_keys[idx]), new_options[''+propt]);
		idx ++;
	}
	test.done();
}
 ...
 * Either run magid directly or run magi-qt with the -server
 * command line option. Make sure you have a ~/.magi/magi.conf
 * with rpcuser and rpcpassword config values filled out. Note that
 * newer versions of magi (1.5 and above) don't allow identical
...
exports.commands_noAuth = {		
	...
	// 			or add "gen=1" to the bottom of your magi.conf file
	getBalance: function(test){
		var curr_balance;
		magi.getBalance(function(err, balance) {
			test.ifError(err);
		  if (err) {
	...
	getGenerate: function(test){
		test.expect(2);
		magi.setGenerate(true,1);	
		test.equal(magi.getGenerate(), true);

		magi.setGenerate(false,1);	
		test.equal(magi.getGenerate(), false);
		test.done();
	},
	getreceived_: function(test){
		var amount= 0.0001;
		//magi.setAccount()
		sendfrom("testnet_user", magi.getaccountaddress('testnet_user'),amount, function(err,addr){
			test.equal(getreceivedbyaccount('testnet_user', amount);
			test.equal( getreceivedbyaddress( magi.getaccountaddress('testnet_user') ), amount);
			test.done();
...
exports.commands_Auth = {		
	...
	setUp: function () {
		magi.auth('testnet_user', 'testnet_pass');
	}
